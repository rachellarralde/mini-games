...
export function AnagramGuesserComponent() {
  ...
  // Move startNewWord and startTimer declarations up
  ...
  // Define resetGame before useEffect
  const resetGame = useCallback(() => {
    ...
    startNewWord();
    startTimer();
  }, [startNewWord, startTimer]);

  // Now use resetGame in useEffect
  useEffect(() => {
    resetGame();
    return () => {
      if (timerRef.current) {
  ...
  const averageTime =
    correctCount > 0 ? Math.round(totalTime / correctCount) : 0;

  return (
    <div className="max-w-md mx-auto mt-8 p-4 font-mono">
      {" "}
      {/* Added font-mono for monospaced font */}
      {gameOver && <Confetti />} {/* Show confetti when the game is won */}
      <div className="hero-section text-center py-8">
        {" "}
        {/* Hero section */}
        <h1 className="text-5xl font-bold mb-8">Anagram Guesser</h1>{" "}
        {/* Hero title */}
      </div>
      <div className="flex justify-between mb-8">
        {" "}
        {/* Timer and correct answers */}
        <p className="text-lg">Time Left: {formatTime(timeLeft)}</p>
        <p className="text-lg">Correct Answers: {correctCount}</p>
      </div>
      <div className="text-center mb-8">
        {" "}
        {/* Added more margin to separate sections */}
      ...
      {gameStatus === "playing" && (
        <div className="text-center mt-4">
          ...
          >
            ...
            {showHint
              ? `Hint: Starts with "${word[0]}" and ends with "${
                  word[word.length - 1]
                }"`
              : "Hint"}
          </p>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div
            className="bg-white p-6 rounded shadow-lg w-80 max-w-md relative z-50"
